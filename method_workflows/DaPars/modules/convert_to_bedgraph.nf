...
process CONVERT_TO_BEDGRAPH {
    ...
    input:
    tuple val(idx), val(sample), val(condition), path(bam_file), path(bai_file)

    output:
    tuple val(idx), val(sample), val(condition), path(bedgraph_file), emit: ch_convert_to_bedgraph_out
