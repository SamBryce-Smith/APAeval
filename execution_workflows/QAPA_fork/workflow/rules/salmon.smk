...
rule salmon_index:
    ...
    params:
        ...
        decoys = "--decoys" if config["add_genome_decoys"] else "",
...
rule salmon_quant_pe:
    ...
    output:
        os.path.join(config["out_dir"], "salmon", "quant", "{sample}", "quant.sf")

    params:
        index_dir = rules.salmon_index.params.outdir,
        output_dir = os.path.join(config["out_dir"], "salmon", "quant", "{sample}"),
